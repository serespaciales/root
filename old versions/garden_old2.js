document.addEventListener('DOMContentLoaded', async () => {
  ...
  function isDefaultSeed(data) {
    // Skip seeds that were never intentionally designed:
    // - no thumbUrl (never saved a snapshot)
    // - no growth applied
    // - all visuals are just the auto-generated random gradients (layers[0] only, no text/shape/image)
    if (!data.layers || data.layers.length === 0) return true;
    const hasGrowth = data.growthConfig &&
      ((data.growthConfig.sun || 0) + (data.growthConfig.water || 0) + (data.growthConfig.vitamins || 0) > 0);
    const hasNonGradient = data.layers.some(layer =>
      layer.visuals && Object.values(layer.visuals).some(v => v.type && v.type !== 'gradient' && v.type !== 'empty')
    );
    const hasMultipleLayers = data.layers.length > 1;
    const hasThumb = !!data.thumbUrl;
    // Consider it intentional if: has growth, has non-gradient visuals, multiple layers, or has a saved thumb
    return !hasGrowth && !hasNonGradient && !hasMultipleLayers && !hasThumb;
  }

  function renderSeeds(docs) {
    docs.forEach(doc => {
      const data = doc.data();
      if (isDefaultSeed(data)) return; // skip untouched seeds
